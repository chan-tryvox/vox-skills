...
- A native custom voice is documented only as the reference `runtime.voice: { "type": "custom", "id": "voice_..." }` for a new GPT-Live custom voice after the organization authorizes it. An arbitrary ID grants no access. Existing pipeline voice settings remain unchanged and are not migrated. Treat custom references as non-executable until authorization and provisioning are complete and quality validation has passed. Do not promise custom provisioning, migration, or exact voice identity.
...
- GPT-Live를 새로 만들 때는 `type: "single_prompt"`와 `data.runtime`, `data.llm.model`을 반드시 포함한다. `type: "flow"`와 `gpt_live` 런타임 조합은 거부되며, 기존 Flow는 `pipeline`을 유지한다. `runtime.voice`는 `builtin`을 실행 설정으로 사용한다. OpenAI 네이티브 `custom`은 새 GPT-Live 커스텀 음성에만 사용하는 참조이며, 기존 pipeline 음성은 그대로 유지하고 마이그레이션하지 않는다. 조직 승인·프로비저닝과 품질 검증이 끝나기 전에는 실행 가능한 설정으로 사용하지 않는다. pipeline용 `stt`/`voice`/`parallelSTT`/`sttPreference`/`voicePreference`와 호환되지 않는 legacy speech preference를 같은 입력에 복사하지 않는다.
